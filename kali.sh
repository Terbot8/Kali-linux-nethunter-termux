...
if [[ \$kali == "0" || ("\$#" != "0" && ("\$1" == "start" || "\$1" == "START")) ]];then
    vncsever -kill :1
    vncserver -kill :2
    vncserver
    if [[ "\$#" != "0" && ("\$1" == "start" || "\$1" == "START") ]];then
        shift
    fi
fi
if [[ \$ubuntu == "0" || ("\$#" != "0" && ("\$1" == "stop" || "\$1" == "STOP")) ]];then
     vncserver -kill :1
     vncserver -kill :2
    if [[ "\$#" != "0" && ("\$1" == "start" || "\$1" == "STOP") ]];then
        shift
    fi
fi
EOM
...
printf "${green}kali vnc start= start vncserver${reset}\n"
printf "${green}kali vnc stop = stop vncserver${reset}\n"
 printf "${green}kali -r vnc start = start vncserver as root${reset}\n"
printf "${green}kali -r vnc stop = stop vncserver as root${reset}\n"
